...
if [[ "${RESIDUAL_MERGE_MODE,,}" == "a2c2" ]]; then
  RESIDUAL_MERGE_MODE="fast_wrist"
fi
FAST_RESIDUAL_ALPHA="${FAST_RESIDUAL_ALPHA:-${A2C2_ALPHA:-1.0}}"
FAST_RESIDUAL_USE_LATENT_CONTEXT="${FAST_RESIDUAL_USE_LATENT_CONTEXT:-${A2C2_USE_LATENT_CONTEXT:-true}}"
...
"$PY" "$PROJECT_ROOT/scripts/train_via_lerobot.py" \
  ...
  --policy.fast_residual_alpha="$FAST_RESIDUAL_ALPHA" \
  --policy.fast_residual_use_latent_context="$FAST_RESIDUAL_USE_LATENT_CONTEXT" \
