...
本研究不是要訓練一個更大的 VLA，也不是要改寫完整 real-time scheduler；而是研究一個較窄但可實驗驗證的問題：若 slow VLA planner 已經凍結，是否能在 action 執行前加入一個小型 fast residual module，用目前腕部相機看到的局部視覺證據修正下一個 base action？
...
這個問題與單純比較模型大小不同。若 \smolvla{} 每一步都重規劃，slow planner 已經能看見最新 observation，fast residual 不一定有幫助，甚至可能破壞原本正確的 action。因此本文評估時必須明確區分 planning horizon、execution horizon 與 replan interval，而不能把 default every-step replanning 當作所有 chunk 設定的 baseline。
...
\begin{quote}
在凍結 \smolvla{} slow planner 的情況下，小型 wrist-camera residual module 是否能改善 frozen VLA action chunk 的執行，特別是在 execution interval 變長或 planner chunk 延遲抵達的 stale-action regime？
\end{quote}
...
\begin{enumerate}
  ...
  \item 在同步 matched chunk sweep 中，當 planning、execution 與 replan interval 均為 $K$ 時，wrist residual correction 是否仍有幫助？
  \item 殘差合併中的 $\alpha$ 與 $\delta_{\max}$ 如何影響成功率？
  \item 在 async-timestep planner-delay stress test 中，fast residual 是否能減緩 chunk late arrival 的退化？
...
\begin{enumerate}
  \item \textbf{凍結 VLA action chunk 的腕部相機殘差修正。} 本研究將 \texttt{HuggingFaceVLA/smolvla\_libero} 固定為 slow planner，只訓練小型 fast residual module，讓研究焦點集中在 execution-time correction。
  \item \textbf{LeRobot-native 研究 artifact。} 本研究建立 \texttt{lerobot\_policy\_hfrvla} policy plugin、HFRVLA LeRobotDataset v3、cached SmolVLA/DINO features、checkpoint packaging 與 eval registry。
  \item \textbf{明確拆分 planning 與 execution protocol。} 所有結果記錄 \texttt{planning\_chunk\_size}、\texttt{execution\_chunk\_size} 與 \texttt{replan\_interval\_steps}，避免錯誤比較不同 runtime contract。
