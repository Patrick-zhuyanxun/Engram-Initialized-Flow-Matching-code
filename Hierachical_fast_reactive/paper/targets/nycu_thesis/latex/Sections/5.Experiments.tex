...
第一個 sweep 固定 planned chunk length 為 50，並令 executed open-loop length 與 replanning interval 共同等於 $K$。此設定測試：當 slow planner 產生長 chunk 時，若 robot 每 $K$ 步才切換或 refresh chunk，fast wrist correction 是否能改善後段 stale actions。
...
Residual calibration sweep 改變 $\alpha$ 與 $\dmax$，觀察 residual authority 對 success rate 的影響。這組實驗回答的問題是：fast wrist correction 應該是有界 local correction，還是可以讓 residual head 自由輸出大幅 action correction。
...
Planner-delay sweep 是本文唯一 async-timestep inference experiment。此實驗中，slow planner 在 step $t$ 發出 request，但 chunk 在 $t+d$ 個 control timesteps 後才可用；execution 從 delayed chunk 的對應 index 開始。Fast wrist correction 仍在 current timestep 執行，因此能利用當前 wrist observation 修正 delayed base action。
...
\begin{itemize}
  \item Dual-view DINO fast correction ablation：同時使用 wrist 與 third-person image 的 fast model。
