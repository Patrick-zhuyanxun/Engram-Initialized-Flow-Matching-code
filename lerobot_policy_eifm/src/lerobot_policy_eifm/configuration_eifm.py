...
@PreTrainedConfig.register_subclass("eifm")
@dataclass
class EIFMConfig(PreTrainedConfig):
    """
    Configuration class for the EI-FM (Engram-Initialized Flow Matching) policy.

    Extends X-VLA with engram-based noise initialization for flow matching.
    When engram_path is None, behaviour is identical to vanilla X-VLA.
    """
    ...
    # ── EI-FM: Engram settings ────────────────────────────────
    engram_path: str | None = None           # Path to engram_table.pt
    engram_p_engram: float = 0.8             # Training: prob of using engram init
    engram_key_mode: str = "multi"           # "single" or "multi"
    train_action_projections: bool = False   # Train action_encoder/decoder
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.engram_p_engram <= 1.0:
            raise ValueError(f"`engram_p_engram` must be in [0, 1], got {self.engram_p_engram}")
    ...
    def validate_features(self) -> None:
        if not self.image_features:
            raise ValueError("EIFM requires at least one visual feature in the inputs.")
        ...
        # LeRobot infers dataset action features before instantiating the policy.
        # For EI-FM, the raw LIBERO action is 7D but the actual policy output is
        # always 20D ee6d after `libero_action_to_ee6d`.
